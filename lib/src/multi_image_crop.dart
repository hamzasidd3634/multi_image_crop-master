...
class _MultiImageCropServiceState extends State<MultiImageCropService>
  ...
  List<CroppedFile> cropFiles = [];
  List<File> simpleCropFiles = [];
  String? mediaType;
  bool isIos = false;
  ...
  PreferredSizeWidget utilityAppBar() {
    return AppBar(
      ...
      elevation: 0,
      backgroundColor: CustomColors.primaryColor,
      leading: widget.isLeading == false
  ...
  cropImage() async {
    if (widget.fromHeroSelection == true) {
      ...
      for (int i = 0; i < files.length; i++) {
        ...
        // _lastCropped?.delete();
        _lastCropped = file;
        simpleCropFiles.add(file);
      }
      Navigator.of(context, rootNavigator: true).pop();
      Navigator.of(context).pop(simpleCropFiles);
    }
  }
